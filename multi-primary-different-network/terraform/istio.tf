...
resource "helm_release" "ztunnel_cluster1" {
  ...
  # Redirect ztunnel's workload xDS to the external gateway deployed in cluster3.
  # The gateway presents a TLS cert signed by the shared root CA; ztunnel
  # verifies it via the istiod-ca-cert ConfigMap (created by local istiod).
  set {
    name  = "xdsAddress"
    value = "10.90.0.2:15012"
  }
  # Keep using local istiod for certificate signing (CA).
  set {
    name  = "caAddress"
  ...
  depends_on = [
    helm_release.istio_cni_cluster1,
    null_resource.discovery_ready,
  ]
}
...
resource "helm_release" "ztunnel_cluster2" {
  ...
  depends_on = [
    helm_release.istio_cni_cluster2,
    null_resource.discovery_ready,
  ]
}
