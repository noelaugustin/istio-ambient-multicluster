...
# -----------------------------------------------------------------------------
# Cluster 3 — MetalLB (discovery cluster)
# IP pool: 10.90.0.x — xds-gateway will be pre-assigned 10.90.0.2
# -----------------------------------------------------------------------------

resource "helm_release" "metallb_cluster3" {
  provider = helm.cluster3
...
resource "null_resource" "metallb_config_cluster3" {
  depends_on = [helm_release.metallb_cluster3]

  triggers = {
    ip_range   = var.cluster3_metallb_ip_range
    cluster_id = kind_cluster.cluster3.id
  }

  provisioner "local-exec" {
    command = <<-EOT
      kubectl --context kind-${var.cluster3_name} apply -f - <<EOF
      apiVersion: metallb.io/v1beta1
      kind: IPAddressPool
      metadata:
        name: default-pool
